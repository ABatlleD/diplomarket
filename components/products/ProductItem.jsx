...
function ProductItem({ title, price, category, image }) {
  return (
    <>
      <div className='flex flex-col hover:cursor-pointer hover:shadow-md my-4 hover:shadow-button w-full border-2 border-background-300 rounded-lg h-64 md:h-72 xl:h-[28rem]'>
        <div className='my-2 w-100 flex flex-row justify-center h-16 md:h-28 xl:h-36'>
          <img src={image} className="max-w-max h-full" alt="..." />
        </div>
        <div className='mx-2 my-2 text-text-100 h-14'>{title}</div>
        <div className='mx-2 my-2 text-button'>{category}</div>
        <div className='mx-2 my-2 text-button font-bold'>$ {price} USD</div>
        ...
        <div className='flex flex-row justify-between mx-2'>
          ...
          >
            <span><AddShoppingCartIcon /></span> Add Cart
          </div>
