...
function FavItem({ id }) {
  ...
  useEffect(() => {
    resources.products.one(id).then((response) => {
            ...
            {product.id && (
              <Link href={`/products/${product.id}`}>
                {resizeTitle(
                  ...
                  size.width > 768 ? (size.width > 1900 ? 60 : 50) : 18
                )}
              </Link>
            )}
          </div>
            ...
            <>
              ...
              {product.promocion && product.promocion.activo && (
                <div className="flex flex-col md:flex-row ml-2 leading-3">
                  <p className="my-0 md:mb-0 md:my-0 text-button font-bold text-xs md:text-base">
                    US$
                    {parseFloat(
                      product.precio -
              ...
              {product.precioxlibra !== 0 && (
                <div className="mx-2 my-0 md:mb-0 md:my-0 text-text-100 text-xs md:text-base">
                  US${parseFloat(product.precioxlibra).toFixed(2)}/{product.um}
                </div>
              )}
              {product.precioxlibra === 0 && <div className="md:h-6"></div>}
            </>
          )}
        </div>
        <Divider
